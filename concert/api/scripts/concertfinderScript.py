...
tagger.open('/home/schlafdata/concertFinderDjango/concert/home/scripts/crf.model')
...
#     matches = matches[['Link','Date','Venue','Caused_By']]
#     matches['Date'] = matches.Date.map(lambda x: x.isoformat())
...
#     temple_events = requests.get('https://www.templedenver.com/event-calendar/all/', verify=False)
#     temple_soup = BeautifulSoup(temple_events.text, "html.parser")
#     event_table = temple_soup.findAll('div', {'class':'uv-calendar-list uv-integration uv-eventlist uv-eventlist-default uv-clearfix'})
...
#     pic = []
#     for x in temple_soup.findAll('div', {'class':'uv-flyer uv-flyerbox'}):
#         try:
#             pic.append('https:'+ x['style'].split('(')[1].split(')')[0])
#         except:
#             pass
...
#     temple_events = pd.DataFrame(list(zip(artist_list,dates_list,link,pic)))
#     temple_events.columns = ['Artist','Date','Link','picLink']
#     temple_events['Venue'] = 'Temple'
...
#     black_soup = BeautifulSoup(black_box.text, 'html.parser')
#     black_event = black_soup.findAll('div', {'class':'list-view-details vevent'})
# #     thumb = black_soup.findAll('div', {'class':'list-view-item tfly-event-id-1870503 tfly-org-id-7601 tfly-venue-id-26153'})
...
#     fox_frame['Link'] = 'No Link at this time, sorry!'
#     fox_frame = fox_frame[['Artist','Date','FiltArtist','Link','Venue']]
#     fox_frame['picLink'] = pics[:len(fox_frame)]
...
#     cervantes_event_frame = cerv_date_frame.merge(cerv_artist_frame, left_on='index', right_on='index', how='inner')
#     cervantes_event_frame = cervantes_event_frame[['Artist','Date','Link']]
#     cervantes_event_frame['picLink'] = pics
...
#         venue = x['venue']['name']
#         link = x['ticketPurchaseUrl']
#         pics = x['image']['small']['path']
#         info = (artists, dates,link, venue, pics)
#         summitInfo.append(info)
...
#         return [x.strip() for x in re.split(',|:', row) if (x is not None) & (x != '')]
#     summitFrame['FiltArtist'] = summitFrame['Artist'].map(splitArtists)
#     summitFrame['Artist'] = summitFrame['Artist'].map(lambda x : x.rstrip(','))
...
def fox_scrape():
    ...
    fox = requests.get('https://www.foxtheatre.com/calendar/', proxies = proxies, verify = False)
    fox_soup = BeautifulSoup(fox.text, 'html.parser')
    calendar= fox_soup.find('div', {'class':'tfly-calendar'})
    tableRows = calendar.findAll('tr')
    #find all table rows
    events = []
    for row in tableRows:
        ...
        for event in column:
            try:
                ...
                eventLinks = ['https://foxtheatre.com' + x['href'] for x in event.findAll('a', {'class':'url'})]
                picLinks = [x['src'] for x in event.findAll('img')]
                venue = [x.contents[0] for x in event.findAll('abbr')]

                eventInfo = list(zip(eventTitle, eventLinks, picLinks, venue))
    ...
    fox_frame.columns = ['Date','Artist','Link','picLink','Venue']
...
def cervantes_scrape():
    ...
    cervantes = requests.get('https://www.cervantesmasterpiece.com/calendar/', proxies = proxies, verify=False)
    cerv_soup = BeautifulSoup(cervantes.text, 'html.parser')
    calendar = cerv_soup.find('div', {'class':'tfly-calendar'})
    tableRows = calendar.findAll('tr')
    #find all table rows
    events = []
    for row in tableRows:
        ...
        for event in column:
            try:
                date = [event.find('span', {'class':'value-title'})['title']]
                eventTitle = [x['alt'] for x in event.findAll('img')]
                eventLinks = ['https://cervantesmasterpiece.com' + x['href'] for x in event.findAll('a', {'class':'url'})]
                picLinks = [x['src'] for x in event.findAll('img')]
                venue = [x.contents[0] for x in event.findAll('abbr')]
    ...
    cervantes_event_frame.columns = ['Date','Artist','Link','picLink','Venue']
